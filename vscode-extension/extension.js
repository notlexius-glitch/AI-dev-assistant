...
// HTTP helper
// ---------------------------------------------------------------------------
function postToApi(endpoint, body, timeoutS) {
    const cfg = vscode.workspace.getConfiguration('qyverixai');
    const baseUrl = cfg.get('apiUrl', 'https://qyverixai.onrender.com').replace(/\/+$/, '');
...
function renderAnalyzeHtml(res) {
...
function renderExplainHtml(res) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
function setDiagnostics(uri, issues) {
    const diagnostics = [];
    for (const issue of issues) {
        ...
        const diag = new vscode.Diagnostic(range, `${issue.type}: ${issue.description}`, severityToVsCode(issue.severity));
...
async function handleAnalyze() {
    ...
    const panel = vscode.window.createWebviewPanel('qyverixai.analyze', `QyverixAI: ${editor.document.fileName.split(/[/\\]/).pop()}`, vscode.ViewColumn.Beside, { enableScripts: false });
    panel.webview.html = loadingHtml('Analyzing code...');
    const lang = editor.document.languageId;
    const timeout = vscode.workspace.getConfiguration('qyverixai').get('timeout', 30);
    try {
        const res = await postToApi('/analyze/', { code, language: lang }, timeout);
        panel.webview.html = renderAnalyzeHtml(res);
        setDiagnostics(editor.document.uri, res.debugging.issues);
        vscode.window.showInformationMessage(`QyverixAI: ${res.debugging.error_count} errors, ${res.debugging.warning_count} warnings — grade ${res.suggestions.grade}`);
    }
    catch (err) {
...
async function handleDebug() {
    ...
    const panel = vscode.window.createWebviewPanel('qyverixai.debug', `QyverixAI Debug: ${editor.document.fileName.split(/[/\\]/).pop()}`, vscode.ViewColumn.Beside, { enableScripts: false });
    panel.webview.html = loadingHtml('Debugging code...');
    const lang = editor.document.languageId;
    const timeout = vscode.workspace.getConfiguration('qyverixai').get('timeout', 30);
    try {
        const res = await postToApi('/debugging/', { code, language: lang }, timeout);
        ...
        if (res.clean) {
            ...
            vscode.window.showWarningMessage(`QyverixAI: ${res.error_count} errors, ${res.warning_count} warnings found`);
        }
    }
    catch (err) {
        panel.webview.html = errorHtml(err.message);
...
async function handleExplain() {
    ...
    const panel = vscode.window.createWebviewPanel('qyverixai.explain', `QyverixAI Explain: ${editor.document.fileName.split(/[/\\]/).pop()}`, vscode.ViewColumn.Beside, { enableScripts: false });
    panel.webview.html = loadingHtml('Generating explanation...');
    const lang = editor.document.languageId;
    const timeout = vscode.workspace.getConfiguration('qyverixai').get('timeout', 30);
    try {
        const res = await postToApi('/explanation/', { code, language: lang }, timeout);
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('qyverixai.analyze', handleAnalyze), vscode.commands.registerCommand('qyverixai.debug', handleDebug), vscode.commands.registerCommand('qyverixai.explain', handleExplain));
}
function deactivate() {
